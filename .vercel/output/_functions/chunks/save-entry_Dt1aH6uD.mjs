...
const POST = async ({ request, cookies, redirect }) => {
  ...
  let error;
  if (id) {
    const result = await client.from("time_entries").update({ date, check_in: checkInTime, check_out: checkOutTime, note }).eq("id", id);
    error = result.error;
  } else {
    const { data: existing } = await client.from("time_entries").select("id").eq("employee_id", employeeId).eq("date", date).single();
    if (existing) {
      const result = await client.from("time_entries").update({ check_in: checkInTime, check_out: checkOutTime, note }).eq("id", existing.id);
      error = result.error;
    } else {
      const result = await client.from("time_entries").insert({ employee_id: employeeId, date, check_in: checkInTime, check_out: checkOutTime, note });
      error = result.error;
    }
  }
  if (error) {
